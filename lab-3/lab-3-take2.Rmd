...
Because there are a fairly large number of predictors here, we set `diag = F` and `type = 'lower'`, which simplifies the plot a bit and may make it easier to read. We can practice interpreting these correlations; for example, `PriceMM` and `PriceCH` are positively correlated. This implies that higher prices for one brand of orange juice are correlated with higher prices for the other, and makes sense; when orange juice in general is cheaper to make, both brands would lower their prices, and vice versa.
...
The orange juice data set is definitely unbalanced; the Citrus Hill brand appears to be more popular overall, with 653 observations representing customers purchasing Citrus Hill and only 417 Minute Maid. For now, we won't adjust for this, but we'll discuss methods of **upsampling** and **downsampling** in next week's lab.
...
```{r}
log_reg_acc <- augment(smarketlog_fit, new_data = smarket_train) %>%
...
And again we can view a confidence matrix and calculate accuracy on the **training data**:
...
Since we've chosen a model, however -- naive Bayes -- we can view its confusion matrix using the **testing** data:
