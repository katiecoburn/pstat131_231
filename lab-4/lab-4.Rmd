...
Earlier, we set `neighbors = 5` to create a second-degree polynomial regression model. But suppose we want to find the best value of `neighbors` that yields the best performance when applied to new data. The process of identifying the best value to set a hyperparameter to is known as hyperparameter **tuning**, and it is a case where we can use k-fold cross-validation.
